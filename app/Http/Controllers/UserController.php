        ...
        if ($request->ajax()) {
            $data = User::join('model_has_roles', 'model_has_roles.model_id', '=', 'users.id')
                ...
                ->addColumn('action', function($row){
                    ...
                    $btn .= '<a href="' . route('user.show', $row->id) . '" class="btn btn-info btn-sm mx-1"><i class="fas fa-eye"></i></a>';
                    $btn .= '<a href="' . route('user.edit', $row->id) . '" class="btn btn-warning btn-sm mx-1"><i class="fas fa-edit"></i></a>';
                    $btn .= '<a href="' . route('user.destroy', $row->id) . '" class="btn btn-danger btn-sm mx-1" onclick="event.preventDefault(); if(confirm(\'Are you sure?\')) { document.getElementById(\'delete-form-' . $row->id . '\').submit(); }"><i class="fas fa-trash"></i></a>';
